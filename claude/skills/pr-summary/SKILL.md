...
現在のブランチに含まれる変更（=PR対象の差分）から、**日本語のPRタイトル**と**MarkdownのPR概要**を作成して提示する。
...
- **1行・70文字以内**を目安に、変更の本質（why/何が変わるか）を一言で表す日本語のタイトルを作る
- リポジトリの慣習を**必ず先に確認**してから合わせる：
  - `gh pr list --state all --limit 20` で直近 PR タイトルを見て、prefix（`fix:` / `feat:` / `add:` / `update:` / `chore:` 等）の有無や日本語/英語の傾向を踏襲する
  - `git log <merge-base>..HEAD --pretty=format:'%s'` の傾向（コミット先頭が `add:` 等で始まっているか）も参考にする
  - 慣習が読み取れないときは prefix なしの素の日本語タイトルにする
- 単一 issue が紐づくときは、issue タイトルを下敷きにしつつ「結局このPRで何が変わるか」を主語にする
- 複数領域に渡る変更は **代表的な変更1つに絞る**（網羅しようとして長くならない）。本文の「変更内容」で詳細を書く
- 例:
  - `fix: 売上レポートAPIのN+1を解消しレスポンス時間を改善`
  - `feat: 展示会フィルタの絞り込み条件にカテゴリを追加`
  - `add: Claude/Codex向け共有hook設定`
...
- まず **PR タイトル** を `**Title:** 〜` の形式で1行で提示する
- 続けて PR 概要を **コードフェンスで囲んだ Markdown ブロック**として一括提示する（そのまま GitHub に貼れる形）
- 不確実な点（背景の推測、影響範囲の不明点）は本文中に「**要確認**」マークで残し、提示後に箇条書きで質問を添える

### 6. ファイル保存（明示指示があった場合のみ）

- ユーザーが「`.local/issues/<NNN>/pr.md` に保存して」のように指示した場合のみ Write
- 既存ファイルがあれば必ず Read してから上書きの可否を確認
...
**Title:** `fix: 売上レポートAPIのN+1を解消しレスポンス時間を改善`
