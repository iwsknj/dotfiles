---
name: diff-review
description: 差分が大きいときに明示的に呼ぶ、解説つきHTMLレビュー画面の生成スキル。差分を意図ごとにグループ分けし、リスク順に並べ、各hunkに解説を付けた自己完結HTMLを `.local/reviews/` に出力する。忖度対策として「planを知らないブラインドレビュー→plan照合」の2段階でレビューし、画面上で指摘の採用/却下・コメント記入・フィードバックmarkdownのコピーができる。トリガー: "/diff-review", "解説つきレビュー", "レビュー画面を作って", "diff review"。
---
...
スキルのファイル: `~/.codex/skills/diff-review/`（以下 `<skill>`）
...
```bash
git diff > /tmp/diff-review/review.patch   # 範囲に応じて（vs branchなら merge-base 起点の三点diff）
python3 <skill>/scripts/split_diff.py /tmp/diff-review/review.patch > /tmp/diff-review/hunks.json
```
...
plan や本セッションの文脈を持たない**別プロセスの Codex** にレビューさせる。「planに則っているから良し」という判断を構造的に不可能にするのが目的。

```bash
codex exec -c 'approval_policy="never"' --sandbox read-only --cd <repo-root> \
  "<下記の指示 + hunks.jsonのパス>" > /tmp/diff-review/stage1.md
```

指示に含めること:

- 差分（hunks.json）だけを根拠にレビューする。周辺コードを読むのは可。ただし `.local/` 配下・plan.md・research.md・PR説明など**意図を説明するドキュメントは読まない**
- 差分から**意図がつかめない変更は、それ自体を「意図不明」として指摘**する（差分だけで意図が伝わらない＝説明かコードに問題がある）
- 最終出力は findings の JSON 配列のみ: `[{severity, title, location, hunkId, body, suggestion}]`（severity: `error`/`warn`/`info`）。前置き・確認は不要

出力ファイルから JSON 部分を回収して使う。
...
下記スキーマで `/tmp/diff-review/annotations.json` を書き、ビルドする。**全 hunk をいずれかのグループに割り当てる**（漏れは build_report.py が警告し「未分類」グループに落ちる。警告が出たら割り当てを直す）。

```bash
python3 <skill>/scripts/build_report.py <skill>/assets/template.html \
  /tmp/diff-review/hunks.json /tmp/diff-review/annotations.json \
